...
async function getConfig() {
  try {
    const r = await br.storage.local.get(['supabaseUrl', 'supabaseAnonKey', 'tmdbApiKey', 'introdbApiKey']);
    return {
    ...
    return { supabaseUrl: null, supabaseAnonKey: null, tmdbApiKey: null, introdbApiKey: null };
  }
}
...
async function checkSupabase(supabaseUrl, supabaseAnonKey) {
  ...
  try {
    ...
    if (res.status === 404 || res.status === 400) {
      return { ok: false, needsManualSetup: true,
        message: 'Table missing — run supabase_setup.sql in your Supabase SQL Editor once.' };
    }
    if (res.status === 401 || res.status === 403) {
      return { ok: false, message: 'Invalid credentials — check your URL and anon key.' };
    }
...
br.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (msg.type === 'TMDB_TO_IMDB') {
    ...
    getConfig().then(({ tmdbApiKey }) => {
      ...
      fetch(`https://api.themoviedb.org/3/tv/${msg.tmdbId}/external_ids?api_key=${tmdbApiKey}`)
        .then(r => r.json())
        .then(data => {
  ...
  // Fallback title→IMDB lookup via OMDB (public demo key; last resort after TMDB path fails)
  if (msg.type === 'OMDB_LOOKUP') {
    if (!msg.title) { sendResponse({ imdbId: null }); return true; }
    fetch(`https://www.omdbapi.com/?t=${encodeURIComponent(msg.title)}&type=series&apikey=trilogy`)
  ...
  if (msg.type === 'SUPABASE_UPSERT') {
    getConfig().then(({ supabaseUrl, supabaseAnonKey }) => {
      ...
      fetch(`${supabaseUrl}/rest/v1/playback_states?on_conflict=user_id,media_id`, {
        method: 'POST',
        headers: {
          apikey: supabaseAnonKey,
          Authorization: `Bearer ${supabaseAnonKey}`,
  ...
  if (msg.type === 'SUPABASE_GET') {
    getConfig().then(({ supabaseUrl, supabaseAnonKey }) => {
      ...
      fetch(url, { headers: { apikey: supabaseAnonKey, Authorization: `Bearer ${supabaseAnonKey}` } })
        .then(r => r.json())
